...
books.forEach(book => {
    book.addEventListener('click', () => {
        const bgImage = getComputedStyle(book).getPropertyValue('--bg-image').trim();
        const url = bgImage.slice(4, -1).replace(/["']/g, ""); 

        modalImg.src = url;
        modalTitle.textContent = book.dataset.title;
        modalAuthor.textContent = book.dataset.author;
        modalDesc.textContent = book.dataset.desc;
        modalYear.textContent = book.dataset.year;
        modalGenre.textContent = book.dataset.genre;
        modalPages.textContent = book.dataset.pages;
